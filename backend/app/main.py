...
# Time Off endpoints (canonical: /time-off; alias: /timeoff)
app.include_router(time_off.router, prefix=f"{API_PREFIX}/time-off")
app.include_router(time_off.router, prefix=f"{API_PREFIX}/timeoff", include_in_schema=False)
# Phase 6: Salary Structures & Rules (canonical: /salary-structures, /salary-rules; alias: /salary/...)
app.include_router(salary_structures.router, prefix=API_PREFIX)
app.include_router(salary_rules.router, prefix=API_PREFIX)
app.include_router(salary_structures.router, prefix=f"{API_PREFIX}/salary", include_in_schema=False)
app.include_router(salary_rules.router, prefix=f"{API_PREFIX}/salary", include_in_schema=False)
# Phase 7: Payruns & Payslips (canonical: /payruns, /payslips; alias: /payroll/...)
app.include_router(payruns.router, prefix=API_PREFIX)
app.include_router(payslips.router, prefix=API_PREFIX)
app.include_router(payruns.router, prefix=f"{API_PREFIX}/payroll", include_in_schema=False)
app.include_router(payslips.router, prefix=f"{API_PREFIX}/payroll", include_in_schema=False)
